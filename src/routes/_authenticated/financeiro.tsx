...
function FinanceiroPage() {
  ...
  const [escopoAtivo, setEscopoAtivo] = useState<"pessoal" | "obra">("pessoal");
  ...
  const [f, setF] = useState<any>({ tipo: "pagar", status: "pendente", escopo: "pessoal" });
  ...
  const contas = escopoAtivo === "pessoal" ? contasPessoais : contasObra;
  ...
  const contasFiltradas = useMemo(
    () =>
      contas.filter((c: any) => {
        ...
        if (
          filtro.categoria &&
          !(c.categoria ?? "").toLowerCase().includes(filtro.categoria.toLowerCase())
        )
          return false;
        return true;
      }),
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      if (payload.escopo === "pessoal") {
        payload.user_id = user?.id;
        payload.obra_id = null;
      } else {
        payload.obra_id = obraId ?? null;
      }
      const { error } = await (supabase as any).from("contas_financeiras").insert(payload);
  ...
  const fluxo = useMemo(() => {
    ...
    const totalPagar = pagas
      .filter((c: any) => c.tipo === "pagar")
      .reduce((s: number, c: any) => s + Number(c.valor), 0);
    const totalReceber = pagas
      .filter((c: any) => c.tipo === "receber")
      .reduce((s: number, c: any) => s + Number(c.valor), 0);
    const pendPagar = contasFiltradas
      .filter((c: any) => c.tipo === "pagar" && c.status !== "pago")
      .reduce((s: number, c: any) => s + Number(c.valor), 0);
    const pendReceber = contasFiltradas
      .filter((c: any) => c.tipo === "receber" && c.status !== "pago")
      .reduce((s: number, c: any) => s + Number(c.valor), 0);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Minhas finanças (privadas) e contas compartilhadas da obra.
        </p>
      </div>
      ...
      <Tabs
        value={escopoAtivo}
        onValueChange={(v) => {
          setEscopoAtivo(v as any);
          setF((p: any) => ({ ...p, escopo: v }));
        }}
      >
        <TabsList>
          <TabsTrigger value="pessoal">Minhas finanças ({contasPessoais.length})</TabsTrigger>
          <TabsTrigger value="obra">Obra ({contasObra.length})</TabsTrigger>
        </TabsList>

        <TabsContent value={escopoAtivo} className="space-y-4 mt-4">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Card className="p-4">
              <p className="text-xs text-muted-foreground">A pagar (pendente)</p>
              <p className="text-2xl font-bold text-destructive">R$ {fluxo.pendPagar.toFixed(2)}</p>
            </Card>
            <Card className="p-4">
              <p className="text-xs text-muted-foreground">A receber (pendente)</p>
              <p className="text-2xl font-bold text-success">R$ {fluxo.pendReceber.toFixed(2)}</p>
            </Card>
            <Card className="p-4">
              <p className="text-xs text-muted-foreground">Saldo realizado</p>
              <p className="text-2xl font-bold">R$ {fluxo.saldo.toFixed(2)}</p>
            </Card>
            <Card className="p-4">
            ...
            <div className="grid gap-2 md:grid-cols-6">
              <div>
                ...
                <Input
                  type="date"
                  value={filtro.ini}
                  onChange={(e) => setFiltro({ ...filtro, ini: e.target.value })}
                />
              </div>
              <div>
              ...
              <div>
                ...
                <Select
                  value={filtro.tipo}
                  onValueChange={(v) => setFiltro({ ...filtro, tipo: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                ...
                <Select
                  value={filtro.status}
                  onValueChange={(v) => setFiltro({ ...filtro, status: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="flex items-end gap-2">
                <Button variant="outline" size="sm" onClick={() => exportContas("csv")}>
                  <FileDown className="h-4 w-4" /> CSV
                </Button>
                <Button variant="outline" size="sm" onClick={() => exportContas("pdf")}>
          ...
          {(canEdit || escopoAtivo === "pessoal") && (
            <Dialog
              open={open}
              onOpenChange={(o) => {
                setOpen(o);
                if (o) setF((p: any) => ({ ...p, escopo: escopoAtivo }));
              }}
            >
              <DialogTrigger asChild>
                <Button>
                  <Plus className="h-4 w-4" /> Nova conta{" "}
                  {escopoAtivo === "pessoal" ? "pessoal" : "da obra"}
                </Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>
                    Nova conta {f.escopo === "pessoal" ? "pessoal" : "da obra"}
                  </DialogTitle>
                </DialogHeader>
                <form
                  onSubmit={(e) => {
                ...
                >
                  ...
                  {f.escopo === "obra" && canEdit && (
                    <div className="space-y-1">
                      <Label>Carteira (usuário)</Label>
                      <Select
                        value={f.user_id ?? ""}
                        onValueChange={(v) => setF({ ...f, user_id: v || null })}
                      >
                        <SelectTrigger>
              ...
              return (
                <TabsContent key={tab} value={tab} className="space-y-2">
                  {list.map((c: any) => {
                    ...
                    const podeExcluir = escopoAtivo === "pessoal" ? isOwner : canDelete;
                    return (
                      <Card key={c.id} className="p-3 flex items-center justify-between">
                        <div>
                          <p className="text-sm font-medium">{c.descricao}</p>
                          <p className="text-xs text-muted-foreground">
                            Venc: {c.data_vencimento} · R$ {Number(c.valor).toFixed(2)} ·{" "}
                            {c.categoria ?? "—"}
                            {atrasado && (
                              <span className="ml-2 text-destructive inline-flex items-center gap-1">
                                <AlertTriangle className="h-3 w-3" /> {Math.abs(dias)}d em atraso
                              </span>
                            )}
                            {!atrasado && c.status !== "pago" && dias <= 7 && (
                              <span className="ml-2 text-warning">vence em {dias}d</span>
                            )}
                          </p>
                        </div>
                        <div className="flex items-center gap-2">
                          <span
                            className={`text-xs px-2 py-1 rounded ${c.status === "pago" ? "bg-success/15 text-success" : atrasado ? "bg-destructive/15 text-destructive" : "bg-warning/15 text-warning"}`}
                          >
                            {c.status}
                          </span>
                          {podeMarcar && c.status !== "pago" && (
                            <Button
                              size="icon"
                              variant="ghost"
                          ...
                          {podeExcluir && (
                            <Button
                              size="icon"
                              variant="ghost"
                              onClick={() => confirm("Excluir?") && remove.mutate(c.id)}
                            >
