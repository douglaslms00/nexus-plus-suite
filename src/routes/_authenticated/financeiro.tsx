...
function FinanceiroPage() {
  ...
  const { data: contasObra = [] } = useQuery({
    ...
    queryFn: async () => {
      let q = (supabase as any).from("contas_financeiras").select("*").eq("escopo", "obra").order("data_vencimento");
  ...
  const { data: contasPessoais = [] } = useQuery({
    ...
    enabled: !!user?.id,
    queryFn: async () => (await (supabase as any).from("contas_financeiras")
      .select("*").eq("escopo", "pessoal").eq("user_id", user!.id).order("data_vencimento")).data ?? [],
  });
  ...
  const marcarPago = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("contas_financeiras").update({ status: "pago", data_pagamento: new Date().toISOString().slice(0, 10) }).eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["contas-obra"] });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Minhas finanças (privadas) e contas compartilhadas da obra.</p>
      </div>

      <Tabs value={escopoAtivo} onValueChange={(v) => { setEscopoAtivo(v as any); setF((p: any) => ({ ...p, escopo: v })); }}>
        <TabsList>
        ...
        <TabsContent value={escopoAtivo} className="space-y-4 mt-4">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Card className="p-4"><p className="text-xs text-muted-foreground">Total lançamentos</p><p className="text-2xl font-bold">{contas.length}</p></Card>
          </div>
          ...
          {(canEdit || escopoAtivo === "pessoal") && (
            <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (o) setF((p: any) => ({ ...p, escopo: escopoAtivo })); }}>
              <DialogTrigger asChild><Button><Plus className="h-4 w-4" /> Nova conta {escopoAtivo === "pessoal" ? "pessoal" : "da obra"}</Button></DialogTrigger>
              <DialogContent>
                <DialogHeader><DialogTitle>Nova conta {f.escopo === "pessoal" ? "pessoal" : "da obra"}</DialogTitle></DialogHeader>
                    ...
                    return (
                      <Card key={c.id} className="p-3 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {podeMarcar && c.status !== "pago" && <Button size="icon" variant="ghost" onClick={() => marcarPago.mutate(c.id)}><Check className="h-4 w-4" /></Button>}
                          {podeExcluir && <Button size="icon" variant="ghost" onClick={() => confirm("Excluir?") && remove.mutate(c.id)}><Trash2 className="h-4 w-4" /></Button>}
                        </div>
                      </Card>
                    );
                  })}
