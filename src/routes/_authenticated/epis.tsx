...
function EpisPage() {
  ...
  const { data: movs = [] } = useQuery({
    queryKey: ["epi_movs"],
    queryFn: async () => {
      const { data, error } = await supabase.from("epi_movimentos")
  ...
  const createEpi = useMutation({
    ...
    onSuccess: () => { toast.success("EPI cadastrado"); qc.invalidateQueries({ queryKey: ["epis"] }); qc.invalidateQueries({ queryKey: ["dash-epis"] }); setOpen(false); setForm({ tipo: "EPI", estoque_atual: 0, estoque_minimo: 0 }); },
  ...
  const createMov = useMutation({
    mutationFn: async () => {
      ...
      if (epi) {
        ...
        const { error: err2 } = await supabase.from("epis")
          .update({ estoque_atual: Math.max(0, (epi.estoque_atual ?? 0) + delta) })
          .eq("id", epi.id);
        if (err2) throw err2;
      }
    },
    onSuccess: () => { toast.success("Movimentação registrada"); qc.invalidateQueries({ queryKey: ["epis"] }); qc.invalidateQueries({ queryKey: ["epi_movs"] }); qc.invalidateQueries({ queryKey: ["dash-epis"] }); setMovOpen(false); setMovForm({ tipo: "entrada", quantidade: 1 }); },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  const removeEpi = useMutation({
    ...
    onSuccess: () => { toast.success("EPI removido"); qc.invalidateQueries({ queryKey: ["epis"] }); },
  });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Cadastro de equipamentos, controle de entrega e estoque.</p>
        </div>
        <div className="flex gap-2">
          {canEdit && (
            <>
              <Dialog open={movOpen} onOpenChange={setMovOpen}>
                <DialogTrigger asChild><Button variant="outline"><ArrowDownToLine className="h-4 w-4" /> Movimentar</Button></DialogTrigger>
                <DialogContent>
                  <DialogHeader><DialogTitle>Movimentação de EPI</DialogTitle></DialogHeader>
                  <form onSubmit={(e) => { e.preventDefault(); createMov.mutate(); }} className="space-y-3">
                    <div className="space-y-1">
                      <Label>EPI / EPC *</Label>
                      <Select value={movForm.epi_id ?? ""} onValueChange={(v) => setMovForm({ ...movForm, epi_id: v })}>
                        <SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger>
                        <SelectContent>{epis.map((e: any) => <SelectItem key={e.id} value={e.id}>{e.nome} ({e.tipo})</SelectItem>)}</SelectContent>
                      </Select>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="space-y-1"><Label>Quantidade</Label><Input type="number" min={1} value={movForm.quantidade} onChange={(e) => setMovForm({ ...movForm, quantidade: e.target.value })} /></div>
                    </div>
                    <div className="space-y-1">
                      <Label>Funcionário (entrega/devolução)</Label>
                      <Select value={movForm.funcionario_id ?? ""} onValueChange={(v) => setMovForm({ ...movForm, funcionario_id: v })}>
              ...
              <Dialog open={open} onOpenChange={setOpen}>
                <DialogTrigger asChild><Button><Plus className="h-4 w-4" /> Novo EPI</Button></DialogTrigger>
                <DialogContent>
                  <DialogHeader><DialogTitle>Cadastrar EPI / EPC</DialogTitle></DialogHeader>
                  <form onSubmit={(e) => { e.preventDefault(); createEpi.mutate(); }} className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="space-y-1"><Label>CA</Label><Input value={form.ca ?? ""} onChange={(e) => setForm({ ...form, ca: e.target.value })} /></div>
                    </div>
                    <div className="grid grid-cols-3 gap-3">
                      <div className="space-y-1"><Label>Estoque atual</Label><Input type="number" min={0} value={form.estoque_atual} onChange={(e) => setForm({ ...form, estoque_atual: e.target.value })} /></div>
                      <div className="space-y-1"><Label>Mínimo</Label><Input type="number" min={0} value={form.estoque_minimo} onChange={(e) => setForm({ ...form, estoque_minimo: e.target.value })} /></div>
                      <div className="space-y-1"><Label>Validade (meses)</Label><Input type="number" min={1} value={form.validade_meses ?? ""} onChange={(e) => setForm({ ...form, validade_meses: e.target.value })} /></div>
                    </div>
                    <DialogFooter><Button type="submit" disabled={createEpi.isPending}>Salvar</Button></DialogFooter>
                  </form>
                </DialogContent>
              </Dialog>
            </>
                ...
                {epis.map((e: any) => {
                  ...
                  return (
                    <TableRow key={e.id}>
                      ...
                      <TableCell><span className="text-xs px-2 py-0.5 rounded bg-muted">{e.tipo}</span></TableCell>
                      <TableCell>{e.ca ?? "—"}</TableCell>
                      <TableCell className={cn(baixo && "text-destructive font-semibold")}>{e.estoque_atual}</TableCell>
                      <TableCell>{e.estoque_minimo}</TableCell>
                      <TableCell className="text-right">
                        {canDelete && <Button size="icon" variant="ghost" onClick={() => { if (confirm("Excluir EPI?")) removeEpi.mutate(e.id); }}><Trash2 className="h-4 w-4" /></Button>}
                      </TableCell>
                    </TableRow>
                  );
                })}
                {epis.length === 0 && <TableRow><TableCell colSpan={6} className="text-center py-8 text-muted-foreground">Nenhum EPI cadastrado.</TableCell></TableRow>}
              </TableBody>
            </Table>
          </Card>
        </TabsContent>
                  ...
                  <TableRow key={m.id}>
                    ...
                    <TableCell>
                      <span className={cn("inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded",
                        ...
                        {m.tipo === "saida" ? <ArrowUpFromLine className="h-3 w-3" /> : <ArrowDownToLine className="h-3 w-3" />}
                        {m.tipo}
                      </span>
                    </TableCell>
