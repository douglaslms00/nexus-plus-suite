...
function MateriaisPage() {
  ...
  const exportConsumo = (kind: "csv" | "pdf") => {
    ...
    const rows = consumo.map((c) => [
      c.nome,
      c.entrada.toFixed(2),
      c.saida.toFixed(2),
      (c.entrada - c.saida).toFixed(2),
    ]);
    if (kind === "csv") {
        ...
        <TabsContent value="cat" className="space-y-3">
          {canCreate && (
            <Dialog
              ...
              onOpenChange={(v) => {
                setOpenM(v);
                if (!v) setEditingM(null);
              }}
            >
              <DialogTrigger asChild>
                <Button onClick={openNewM}>
                  <Plus className="h-4 w-4" /> Novo material
                </Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>{editingM ? "Editar material" : "Novo material"}</DialogTitle>
                </DialogHeader>
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
                ...
                >
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <Input
                        required
                        value={fM.nome ?? ""}
                        onChange={(e) => setFM({ ...fM, nome: e.target.value })}
                      />
                    </div>
                    <div className="space-y-1">
                      <Label>Código</Label>
                      <Input
                        value={fM.codigo ?? ""}
                        onChange={(e) => setFM({ ...fM, codigo: e.target.value })}
                      />
                    </div>
                    <div className="space-y-1">
                      <Label>Unidade</Label>
                      <Input
                        value={fM.unidade ?? "un"}
                        onChange={(e) => setFM({ ...fM, unidade: e.target.value })}
                      />
                    </div>
                    <div className="space-y-1">
                      ...
                      <Input
                        type="number"
                        step="0.001"
                        value={fM.estoque_minimo ?? ""}
                        onChange={(e) => setFM({ ...fM, estoque_minimo: e.target.value })}
                      />
                    </div>
                    <div className="space-y-1">
                  ...
                  <DialogFooter>
                    <Button type="submit" disabled={saveMat.isPending}>
                      {editingM ? "Salvar" : "Criar"}
                    </Button>
                  </DialogFooter>
                </form>
              </DialogContent>
                    ...
                    <TableRow key={m.id}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          {canCreate && (
                            <Button size="icon" variant="ghost" onClick={() => openEditM(m)}>
                          ...
                          {canDelete && (
                            <Button
                              size="icon"
                              variant="ghost"
                              onClick={() => confirm("Excluir?") && removeMat.mutate(m.id)}
                            >
        ...
        <TabsContent value="mov" className="space-y-3">
          <div className="flex flex-wrap gap-2 items-end justify-between">
                ...
                <DialogContent>
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <Input
                          ...
                          step="0.001"
                          required
                          value={fMv.quantidade ?? ""}
                          onChange={(e) => setFMv({ ...fMv, quantidade: e.target.value })}
                        />
                      </div>
                    </div>
                    <div className="space-y-1">
                      ...
                      <Select
                        value={fMv.obra_id ?? ""}
                        onValueChange={(v) => setFMv({ ...fMv, obra_id: v })}
                      >
                        <SelectTrigger>
                          <SelectValue placeholder="Selecione" />
                        </SelectTrigger>
                    ...
                    <div className="space-y-1">
                      ...
                      <Textarea
                        value={fMv.observacoes ?? ""}
                        onChange={(e) => setFMv({ ...fMv, observacoes: e.target.value })}
                      />
                    </div>
                    <DialogFooter>
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={() => exportMovs("csv")}>
                <FileDown className="h-4 w-4" /> CSV
              </Button>
              <Button variant="outline" size="sm" onClick={() => exportMovs("pdf")}>
          ...
          <Card className="p-3">
            <div className="grid gap-2 md:grid-cols-5">
              ...
              <div>
                ...
                <Select
                  value={filtro.obra}
                  onValueChange={(v) => setFiltro({ ...filtro, obra: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => setFiltro({ ...filtro, material: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-sm font-medium">
                      {m.material?.nome} — {Number(m.quantidade).toFixed(2)} {m.material?.unidade}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {m.data} {m.obra?.nome && `· ${m.obra.nome}`}{" "}
          ...
          <Card className="p-3">
            <div className="grid gap-2 md:grid-cols-4">
              ...
              <div className="flex items-end gap-2">
                <Button variant="outline" size="sm" onClick={() => exportConsumo("csv")}>
                  <FileDown className="h-4 w-4" /> CSV
                </Button>
                <Button variant="outline" size="sm" onClick={() => exportConsumo("pdf")}>
                  <FileText className="h-4 w-4" /> PDF
                </Button>
              </div>
            </div>
              ...
              <tbody>
                {consumo.map((c) => (
                  <tr key={c.nome} className="border-b">
                    ...
                    <td>{c.entrada.toFixed(2)}</td>
                    <td>{c.saida.toFixed(2)}</td>
                    <td>{(c.entrada - c.saida).toFixed(2)}</td>
                  </tr>
                ))}
                {consumo.length === 0 && (
                  <tr>
