...
function FrotaPage() {
  ...
  // ---- QUERIES (defensivo: tabela pode não existir antes da migration) ----
  ...
  const { data: veiculos = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        let q: any = supabase.from("frota_veiculos").select("*, obra:obras(nome)").order("placa");
        if (obraId) q = q.eq("obra_id", obraId);
        const { data, error } = await q;
        if (error) {
          console.warn("[frota:veiculos]", error.message);
          return [];
        }
        return data ?? [];
      } catch (e: any) {
        console.warn("[frota:veiculos]", e?.message ?? String(e));
        return [];
      }
    },
  });
  const { data: motoristas = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        if (error) {
          console.warn("[frota:motoristas]", error.message);
          return [];
        }
        return data ?? [];
      } catch (e: any) {
        console.warn("[frota:motoristas]", e?.message ?? String(e));
        return [];
      }
    },
  });
  const { data: abastecimentos = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const { data, error } = await q;
        if (error) {
          console.warn("[frota:abastecimentos]", error.message);
          return [];
        }
        return data ?? [];
      } catch (e: any) {
        console.warn("[frota:abastecimentos]", e?.message ?? String(e));
        return [];
      }
    },
  });
  const { data: manutencoes = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const { data, error } = await q;
        if (error) {
          console.warn("[frota:manutencoes]", error.message);
          return [];
        }
        return data ?? [];
      } catch (e: any) {
        console.warn("[frota:manutencoes]", e?.message ?? String(e));
        return [];
      }
    },
  });
  const { data: gastos = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const { data, error } = await q;
        if (error) {
          console.warn("[frota:gastos]", error.message);
          return [];
        }
        return data ?? [];
      } catch (e: any) {
        console.warn("[frota:gastos]", e?.message ?? String(e));
        return [];
      }
    },
  });
  const { data: pedagios = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const { data, error } = await q;
        if (error) {
          console.warn("[frota:pedagios]", error.message);
          return [];
        }
        return data ?? [];
      } catch (e: any) {
        console.warn("[frota:pedagios]", e?.message ?? String(e));
        return [];
      }
    },
  });
  const { data: obras = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        return (await supabase.from("obras").select("id, nome").order("nome")).data ?? [];
      } catch {
        return [];
      }
    },
  ...
  const kpis = useMemo(() => {
    ...
    const cnhVenc = motoristas.filter(
      (m: any) => statusCNH(m.cnh_validade).nivel === "vencido",
    ).length;
    const cnhAtencao = motoristas.filter(
      (m: any) => statusCNH(m.cnh_validade).nivel === "atencao",
    ).length;
    return {
      totalAbast,
      totalManut,
      totalGastos,
      totalPed,
      totalGeral: totalAbast + totalManut + totalGastos + totalPed,
      consumoGeral,
      alertas,
      cnhVenc,
      cnhAtencao,
  ...
  const [fV, setFV] = useState<any>({
    status: "ativo",
    tipo: "leve",
    combustivel_padrao: "diesel",
    intervalo_revisao_km: 10000,
    intervalo_revisao_meses: 6,
  });
  ...
  const createVeiculo = useMutation({
    mutationFn: async () => {
      const payload = {
        ...fV,
        created_by: user?.id,
        placa: fV.placa?.toUpperCase().replace(/[^A-Z0-9]/g, ""),
      };
  ...
  const createAbast = useMutation({
    mutationFn: async () => {
      ...
      const payload: any = {
        ...fA,
        valor_total: isNaN(valor_total) ? fA.valor_total : valor_total,
        created_by: user?.id,
        obra_id: fA.obra_id ?? obraId ?? null,
      };
      ...
      if (veic && Number(payload.odometro) > Number(veic.odometro_atual)) {
        await supabase
          .from("frota_veiculos")
          .update({ odometro_atual: Number(payload.odometro) })
  ...
  const createManut = useMutation({
    mutationFn: async () => {
      ...
      if (!payload.veiculo_id || !payload.servico)
        throw new Error("Veículo e serviço obrigatórios");
  ...
  const createPedagio = useMutation({
    mutationFn: async () => {
      ...
      if (!payload.veiculo_id || !payload.praca || !payload.valor)
        throw new Error("Veículo, praça e valor obrigatórios");
  ...
  if (!perm.can_view) {
    return (
      <Card className="p-8 text-center text-muted-foreground">
        Você não tem permissão para visualizar Gestão de Frota.
      </Card>
    );
  }
  ...
  const filteredVeiculos = (veiculos as any[]).filter((v) =>
    !search
      ? true
      : `${v.placa} ${v.modelo} ${v.marca ?? ""}`.toLowerCase().includes(search.toLowerCase()),
  );
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-muted-foreground">
          Custos, eficiência dos veículos e desempenho dos motoristas.
        </p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">
        <Card className="p-4">
          ...
          <p className="text-[11px] text-muted-foreground">
            {filteredVeiculos.filter((v) => v.status === "ativo").length} ativos
          </p>
        </Card>
        <Card className="p-4">
          <p className="text-xs text-muted-foreground flex items-center gap-1">
            <Users className="h-3 w-3" /> Motoristas
          </p>
          <p className="text-2xl font-bold">{motoristas.length}</p>
          <p className="text-[11px] text-muted-foreground">
            {(motoristas as any[]).filter((m) => m.status === "ativo").length} ativos
          </p>
        </Card>
        <Card className="p-4">
          <p className="text-xs text-muted-foreground flex items-center gap-1">
            <Gauge className="h-3 w-3" /> Média Geral
          </p>
          <p className="text-2xl font-bold">
            {kpis.consumoGeral.mediaKml != null
              ? `${kpis.consumoGeral.mediaKml.toFixed(2)} km/L`
              : "—"}
          </p>
          <p className="text-[11px] text-muted-foreground">
            {kpis.consumoGeral.custoPorKm != null
              ? `${formatCurrency(kpis.consumoGeral.custoPorKm)}/km`
              : "sem dados"}
          </p>
        </Card>
        <Card className="p-4">
          <p className="text-xs text-muted-foreground flex items-center gap-1">
            <Receipt className="h-3 w-3" /> Custo Total
          </p>
          <p className="text-2xl font-bold">{formatCurrency(kpis.totalGeral)}</p>
          <p className="text-[11px] text-muted-foreground">
            comb. {formatCurrency(kpis.totalAbast)}
          </p>
        </Card>
        <Card
          className={`p-4 ${kpis.alertas ? "border-amber-300 bg-amber-50/40 dark:bg-amber-950/10" : ""}`}
        >
          <p className="text-xs text-muted-foreground flex items-center gap-1">
            <Wrench className="h-3 w-3" /> Revisões
          </p>
          <p className="text-2xl font-bold">{kpis.alertas}</p>
          <p className="text-[11px] text-muted-foreground">
            {kpis.alertas ? "atenção/vencidas" : "em dia"}
          </p>
        </Card>
        <Card
          className={`p-4 ${kpis.cnhVenc ? "border-rose-300 bg-rose-50/40 dark:bg-rose-950/10" : ""}`}
        >
        ...
        <Card className="p-4">
          <p className="text-xs text-muted-foreground flex items-center gap-1">
            <Waypoints className="h-3 w-3" /> Pedágios
          </p>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-2">
          <TabsList className="flex-wrap h-auto">
            ...
            <TabsTrigger value="combustivel" className="gap-1">
            ...
            <TabsTrigger value="gastos" className="gap-1">
              <Receipt className="h-3.5 w-3.5" /> Gastos Avulsos
            </TabsTrigger>
            <TabsTrigger value="pedagios" className="gap-1">
              <Waypoints className="h-3.5 w-3.5" /> Pedágios
            </TabsTrigger>
            <TabsTrigger value="motoristas" className="gap-1">
        ...
        <TabsContent value="visao" className="space-y-4 mt-4">
          <div className="grid gap-4 lg:grid-cols-2">
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base flex items-center gap-2">
                  <Fuel className="h-4 w-4" /> Eficiência por Veículo (km/L)
                </CardTitle>
              </CardHeader>
              <CardContent>
                {veiculos.length === 0 ? (
                        ...
                        >
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              {Number(v.odometro_atual ?? 0).toLocaleString("pt-BR")} km ·{" "}
                              {a.nivel === "vencido" ? (
                                <span className="text-rose-600 font-semibold">{a.msg}</span>
                              ) : a.nivel === "atencao" ? (
                                <span className="text-amber-600">{a.msg}</span>
                              ) : (
                                <span>{a.msg}</span>
                              )}
                            </p>
                          ...
                          <div className="text-right">
                            <p className="text-sm font-bold">
                              {c?.mediaKml != null ? `${c.mediaKml.toFixed(2)} km/L` : "—"}
                            </p>
                            <p className="text-xs text-muted-foreground">
                              {c?.custoPorKm != null ? `${formatCurrency(c.custoPorKm)}/km` : "—"}
                            </p>
                          </div>
                        </div>
                      );
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base flex items-center gap-2">
                  <Crown className="h-4 w-4 text-amber-500" /> Ranking Motoristas (eficiência)
                </CardTitle>
                <CardDescription className="text-xs">
                  Maior km/L primeiro · considera apenas abastecimentos vinculados
                </CardDescription>
              </CardHeader>
              <CardContent>
                {ranking.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {ranking.slice(0, 8).map((r, idx) => (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <span
                            className={`h-6 w-6 rounded-full flex items-center justify-center text-xs font-bold ${idx === 0 ? "bg-amber-400 text-white" : idx === 1 ? "bg-zinc-400 text-white" : idx === 2 ? "bg-amber-700 text-white" : "bg-muted text-muted-foreground"}`}
                          >
                            {idx + 1}
                          </span>
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              {r.totalAbast} abastec. · {formatCurrency(r.gastoTotal)} total
                            </p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="text-sm font-bold">
                            {r.mediaKml != null ? `${r.mediaKml.toFixed(2)} km/L` : "—"}
                          </p>
                          <p className="text-xs text-muted-foreground">
                            {statusCNH(r.motorista.cnh_validade).label}
                          </p>
                        </div>
                      </div>
                    ))}
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            <Card className="p-4">
              <p className="text-xs text-muted-foreground">Combustível (total)</p>
              <p className="text-xl font-bold">
                {formatCurrency(kpis.totalAbast)} · {(abastecimentos as any[]).length} abastec.
              </p>
            </Card>
            <Card className="p-4">
              <p className="text-xs text-muted-foreground">Manutenções (total)</p>
              <p className="text-xl font-bold">
                {formatCurrency(kpis.totalManut)} · {(manutencoes as any[]).length} regs.
              </p>
            </Card>
            <Card className="p-4">
              <p className="text-xs text-muted-foreground">Avulsos + Pedágios</p>
              <p className="text-xl font-bold">
                {formatCurrency(kpis.totalGastos + kpis.totalPed)} ·{" "}
                {(gastos as any[]).length + (pedagios as any[]).length} regs.
              </p>
            </Card>
          </div>
        </TabsContent>
                ...
                >
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <Input
                        required
                        value={fV.placa ?? ""}
                        onChange={(e) => setFV({ ...fV, placa: e.target.value.toUpperCase() })}
                        placeholder="ABC1234"
                        maxLength={7}
                      />
                    </div>
                    <div className="space-y-1">
                      ...
                      <Input
                        required
                        value={fV.modelo ?? ""}
                        onChange={(e) => setFV({ ...fV, modelo: e.target.value })}
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        value={fV.marca ?? ""}
                        onChange={(e) => setFV({ ...fV, marca: e.target.value })}
                      />
                    </div>
                    <div className="space-y-1">
                      <Label>Ano</Label>
                      <Input
                        ...
                        value={fV.ano ?? ""}
                        onChange={(e) =>
                          setFV({ ...fV, ano: e.target.value ? Number(e.target.value) : null })
                        }
                      />
                    </div>
                    <div className="space-y-1">
                      <Label>Tipo</Label>
                      <Select value={fV.tipo} onValueChange={(v) => setFV({ ...fV, tipo: v })}>
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="space-y-1">
                      ...
                      <Select
                        value={fV.combustivel_padrao}
                        onValueChange={(v) => setFV({ ...fV, combustivel_padrao: v })}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        type="number"
                        required
                        value={fV.odometro_atual ?? ""}
                        onChange={(e) => setFV({ ...fV, odometro_atual: Number(e.target.value) })}
                      />
                    </div>
                    <div className="space-y-1">
                      <Label>Status</Label>
                      <Select value={fV.status} onValueChange={(v) => setFV({ ...fV, status: v })}>
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        type="number"
                        value={fV.odometro_proxima_revisao ?? ""}
                        onChange={(e) =>
                          setFV({
                            ...fV,
                            odometro_proxima_revisao: e.target.value
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        type="date"
                        value={fV.data_proxima_revisao ?? ""}
                        onChange={(e) =>
                          setFV({ ...fV, data_proxima_revisao: e.target.value || null })
                        }
                      />
                    </div>
                    <div className="space-y-1">
                      <Label>Obra</Label>
                      <Select
                        value={fV.obra_id ?? "none"}
                        onValueChange={(v) => setFV({ ...fV, obra_id: v === "none" ? null : v })}
                      >
                        <SelectTrigger>
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        type="number"
                        value={fV.intervalo_revisao_km ?? 10000}
                        onChange={(e) =>
                          setFV({ ...fV, intervalo_revisao_km: Number(e.target.value) })
                        }
                      />
                    </div>
                  </div>
                  <div className="space-y-1">
                    ...
                    <Textarea
                      ...
                      onChange={(e) => setFV({ ...fV, observacoes: e.target.value })}
                    />
                  </div>
                  <DialogFooter>
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {filteredVeiculos.map((v: any) => {
              ...
              return (
                <Card
                  key={v.id}
                  className={`p-4 ${alerta.nivel === "vencido" ? "border-rose-300" : alerta.nivel === "atencao" ? "border-amber-300" : ""}`}
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="font-semibold flex items-center gap-2">
                        {v.placa} <Badge variant="outline">{v.tipo}</Badge>{" "}
                        <Badge
                          variant={
                            v.status === "ativo"
                              ? "default"
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        {Number(v.odometro_atual ?? 0).toLocaleString("pt-BR")} km rodados{" "}
                        {v.obra?.nome ? `· ${v.obra.nome}` : ""}
                      </p>
                      <p
                        className={`text-xs mt-1 ${alerta.nivel === "vencido" ? "text-rose-600 font-semibold" : alerta.nivel === "atencao" ? "text-amber-600" : "text-muted-foreground"}`}
                      >
                        {alerta.nivel !== "ok" && <AlertTriangle className="h-3 w-3 inline mr-1" />}
                        {alerta.msg}
                      </p>
                      <div className="mt-2 flex gap-3 text-xs">
                        <span className="flex items-center gap-1">
                          <TrendingUp className="h-3 w-3" />
                          {consumo?.mediaKml != null ? `${consumo.mediaKml.toFixed(2)} km/L` : "—"}
                        </span>
                        <span className="flex items-center gap-1">
                          <Gauge className="h-3 w-3" />
                          {consumo?.custoPorKm != null
                            ? `${formatCurrency(consumo.custoPorKm)}/km`
                            : "—"}
                        </span>
                      </div>
                    </div>
                    {canDelete && (
                      <Button
                        ...
                        onClick={() =>
                          confirm(
                            "Excluir veículo? Isso apagará abastecimentos e registros vinculados.",
                          ) && removeVeiculo.mutate(v.id)
                        }
                      >
            ...
            {filteredVeiculos.length === 0 && (
              <Card className="p-8 text-center text-muted-foreground md:col-span-3">
        ...
        <TabsContent value="combustivel" className="space-y-3 mt-4">
          <div className="flex flex-wrap gap-2">
                  ...
                  >
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <Select
                          ...
                          onValueChange={(v) => {
                            ...
                            setFA({
                              ...fA,
                              veiculo_id: v,
                              tipo_combustivel: veic?.combustivel_padrao ?? fA.tipo_combustivel,
                            });
                          }}
                        >
                          <SelectTrigger>
                      ...
                      <div className="space-y-1 col-span-2">
                        ...
                        <Select
                          value={fA.motorista_id ?? "none"}
                          onValueChange={(v) =>
                            setFA({ ...fA, motorista_id: v === "none" ? null : v })
                          }
                        >
                          <SelectTrigger>
                            <SelectValue placeholder="Selecione" />
                      ...
                      <div className="space-y-1">
                        ...
                        <Input
                          type="date"
                          required
                          value={fA.data ?? new Date().toISOString().slice(0, 10)}
                          onChange={(e) => setFA({ ...fA, data: e.target.value })}
                        />
                      </div>
                      <div className="space-y-1">
                        ...
                        <Input
                          ...
                          required
                          value={fA.odometro ?? ""}
                          onChange={(e) => setFA({ ...fA, odometro: Number(e.target.value) })}
                        />
                      </div>
                      <div className="space-y-1">
                        <Label>Litros *</Label>
                      ...
                      <div className="space-y-1">
                        ...
                        <Select
                          value={fA.tipo_combustivel}
                          onValueChange={(v) => setFA({ ...fA, tipo_combustivel: v })}
                        >
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
                      ...
                      <div className="space-y-1">
                        ...
                        <Input
                          ...
                          step="0.01"
                          required
                          value={fA.valor_por_litro ?? ""}
                          onChange={(e) => setFA({ ...fA, valor_por_litro: e.target.value })}
                        />
                      </div>
                      <div className="space-y-1">
                        ...
                        <Input
                          disabled
                          value={
                            fA.litros && fA.valor_por_litro
                              ? (Number(fA.litros) * Number(fA.valor_por_litro)).toFixed(2)
                              : ""
                          }
                          placeholder="litros × R$/L"
                        />
                      </div>
                      <div className="space-y-1 col-span-2">
                        <Label>Posto</Label>
                        <Input
                          value={fA.posto ?? ""}
                          onChange={(e) => setFA({ ...fA, posto: e.target.value })}
                        />
                      </div>
                    </div>
                    <DialogFooter>
            ...
            <Button variant="outline" size="sm" onClick={() => exportAbast("csv")}>
              <FileDown className="h-4 w-4" /> CSV
            </Button>
            <Button variant="outline" size="sm" onClick={() => exportAbast("pdf")}>
            ...
            <div className="overflow-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {(() => {
                    const byVeic = new Map<string, any[]>();
                    for (const a of [...(abastecimentos as any[])].sort(
                      (x, y) => x.data.localeCompare(y.data) || x.odometro - y.odometro,
                    )) {
                      const arr = byVeic.get(a.veiculo_id) ?? [];
                      arr.push(a);
                      byVeic.set(a.veiculo_id, arr);
                    ...
                    return (abastecimentos as any[])
                      .filter(
                        (a) =>
                          !search ||
                          `${a.veiculo?.placa} ${a.motorista?.nome ?? ""} ${a.tipo_combustivel}`
                            .toLowerCase()
                            ...
                            <td className="p-2 text-right">
                              {canDelete && (
                                <Button
                                  size="icon"
                                  variant="ghost"
                                  onClick={() => confirm("Excluir?") && removeAbast.mutate(a.id)}
                                >
              ...
              {(abastecimentos as any[]).length === 0 && (
                ...
                <DialogContent>
                  ...
                  >
                    <div className="space-y-1">
                      ...
                      <Select
                        value={fM.veiculo_id ?? ""}
                        onValueChange={(v) => setFM({ ...fM, veiculo_id: v })}
                      >
                        <SelectTrigger>
                          <SelectValue placeholder="Selecione" />
                        </SelectTrigger>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <Select
                          value={fM.servico}
                          onValueChange={(v) => setFM({ ...fM, servico: v })}
                        >
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
                      ...
                      <div className="space-y-1">
                        ...
                        <Input
                          type="date"
                          value={fM.data ?? new Date().toISOString().slice(0, 10)}
                      ...
                      <div className="space-y-1">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setFM({
                              ...fM,
                              odometro: e.target.value ? Number(e.target.value) : null,
                            })
                          }
                        />
                      </div>
                      <div className="space-y-1">
                        ...
                        <Input
                          value={fM.oficina ?? ""}
                          onChange={(e) => setFM({ ...fM, oficina: e.target.value })}
                        />
                      </div>
                      <div className="space-y-1">
                        <Label>Peças trocadas</Label>
                        <Input
                          value={fM.pecas_trocadas ?? ""}
                          onChange={(e) => setFM({ ...fM, pecas_trocadas: e.target.value })}
                        />
                      </div>
                      <div className="space-y-1">
                        ...
                        <Input
                          type="number"
                          step="0.01"
                          value={fM.valor_mao_obra ?? ""}
                          onChange={(e) => setFM({ ...fM, valor_mao_obra: e.target.value })}
                        />
                      </div>
                      <div className="space-y-1">
                        <Label>Peças (R$)</Label>
                        <Input
                          type="number"
                          step="0.01"
                          value={fM.valor_pecas ?? ""}
                          onChange={(e) => setFM({ ...fM, valor_pecas: e.target.value })}
                        />
                      </div>
                      <div className="space-y-1">
                        <Label>Próx. revisão (km)</Label>
                        <Input
                          type="number"
                          value={fM.proxima_revisao_km ?? ""}
                          onChange={(e) =>
                            setFM({
                              ...fM,
                              proxima_revisao_km: e.target.value ? Number(e.target.value) : null,
                            })
                          }
                        />
                      </div>
                      <div className="space-y-1">
                        ...
                        <Input
                          type="date"
                          value={fM.proxima_revisao_data ?? ""}
                          onChange={(e) =>
                            setFM({ ...fM, proxima_revisao_data: e.target.value || null })
                          }
                        />
                      </div>
                    </div>
          ...
          <div className="grid gap-2">
            {(manutencoes as any[])
              .filter(
                (m) =>
                  !search ||
                  `${m.veiculo?.placa} ${m.servico} ${m.oficina ?? ""}`
                    .toLowerCase()
                    .includes(search.toLowerCase()),
              )
              .map((m) => {
                ...
                return (
                  <Card key={m.id} className="p-3 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium">
                          {m.veiculo?.placa} — {m.servico}{" "}
                          <Badge
                            variant={m.tipo === "preventiva" ? "secondary" : "destructive"}
                            className="ml-1"
                          >
                            {m.tipo}
                          </Badge>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {m.data}{" "}
                          {m.odometro ? `· ${Number(m.odometro).toLocaleString("pt-BR")} km` : ""}{" "}
                          {m.oficina ? `· ${m.oficina}` : ""}{" "}
                          {m.pecas_trocadas ? `· ${m.pecas_trocadas}` : ""}
                        </p>
                        {m.proxima_revisao_km || m.proxima_revisao_data ? (
                          <p className="text-xs text-amber-600">
                            ...
                            {m.proxima_revisao_km
                              ...
                              : ""}{" "}
                            {m.proxima_revisao_data ? `· ${m.proxima_revisao_data}` : ""}
                          </p>
                        ) : null}
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      {canDelete && (
                        <Button
                          size="icon"
                          variant="ghost"
                          onClick={() => confirm("Excluir?") && removeManut.mutate(m.id)}
                        >
                          <Trash2 className="h-4 w-4" />
                        </Button>
                      )}
            ...
            {(manutencoes as any[]).length === 0 && (
              <Card className="p-8 text-center text-muted-foreground">Nenhuma manutenção.</Card>
            )}
          </div>
        </TabsContent>
                  ...
                  >
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        ...
                        <Input
                          type="date"
                          value={fG.data ?? new Date().toISOString().slice(0, 10)}
                          onChange={(e) => setFG({ ...fG, data: e.target.value })}
                        />
                      </div>
                      <div className="space-y-1">
                        ...
                        <Select
                          value={fG.categoria}
                          onValueChange={(v) => setFG({ ...fG, categoria: v })}
                        >
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
                      ...
                      <div className="space-y-1">
                        ...
                        <Select
                          value={fG.veiculo_id ?? "none"}
                          onValueChange={(v) =>
                            setFG({ ...fG, veiculo_id: v === "none" ? null : v })
                          }
                        >
                          <SelectTrigger>
                      ...
                      <div className="space-y-1">
                        <Label>Motorista</Label>
                        <Select
                          value={fG.motorista_id ?? "none"}
                          onValueChange={(v) =>
                            setFG({ ...fG, motorista_id: v === "none" ? null : v })
                          }
                        >
                          <SelectTrigger>
                            <SelectValue placeholder="—" />
                      ...
                      <div className="space-y-1 col-span-2">
                        ...
                        <Input
                          required
                          value={fG.descricao ?? ""}
                          onChange={(e) => setFG({ ...fG, descricao: e.target.value })}
                      ...
                      <div className="space-y-1">
                        ...
                        <Input
                          ...
                          required
                          value={fG.valor ?? ""}
                          onChange={(e) => setFG({ ...fG, valor: e.target.value })}
                        />
                      </div>
                      <div className="space-y-1">
                        <Label>Forma pagamento</Label>
                        <Input
                          value={fG.forma_pagamento ?? ""}
                          onChange={(e) => setFG({ ...fG, forma_pagamento: e.target.value })}
                    ...
                    <div className="space-y-1">
                      <Label>Observações</Label>
                      <Textarea
                        value={fG.observacoes ?? ""}
                        onChange={(e) => setFG({ ...fG, observacoes: e.target.value })}
                      />
                    </div>
                    <DialogFooter>
                      <Button type="submit">Salvar</Button>
          ...
          <div className="grid gap-2">
            {(gastos as any[])
              .filter(
                (g) =>
                  !search ||
                  `${g.categoria} ${g.descricao} ${g.veiculo?.placa ?? ""}`
                    .toLowerCase()
                    .includes(search.toLowerCase()),
              )
              .map((g) => (
                <Card key={g.id} className="p-3 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium">
                      {g.descricao} <Badge variant="outline">{g.categoria}</Badge>
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {g.data} {g.veiculo?.placa ? `· ${g.veiculo.placa}` : ""}{" "}
                      {g.motorista?.nome ? `· ${g.motorista.nome}` : ""}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold">{formatCurrency(g.valor)}</span>
                    {canDelete && (
                      <Button
                        size="icon"
                        variant="ghost"
                        onClick={() => confirm("Excluir?") && removeGasto.mutate(g.id)}
                      >
                        <Trash2 className="h-4 w-4" />
                      </Button>
                    )}
            ...
            {(gastos as any[]).length === 0 && (
              <Card className="p-8 text-center text-muted-foreground">Nenhum gasto avulso.</Card>
            )}
          </div>
        </TabsContent>
          ...
          <div className="flex flex-wrap gap-2 items-center">
            {canEdit && (
              <Dialog open={openP} onOpenChange={setOpenP}>
                ...
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Novo pedágio</DialogTitle>
                  </DialogHeader>
                  <form
                    onSubmit={(e) => {
                      e.preventDefault();
                      createPedagio.mutate();
                    }}
                    className="space-y-3"
                  >
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1 col-span-2">
                        ...
                        <Select
                          value={fP.veiculo_id ?? ""}
                          onValueChange={(v) => setFP({ ...fP, veiculo_id: v })}
                        >
                          <SelectTrigger>
                            <SelectValue placeholder="Selecione" />
                          </SelectTrigger>
                      ...
                      <div className="space-y-1">
                        <Label>Motorista</Label>
                        <Select
                          value={fP.motorista_id ?? "none"}
                          onValueChange={(v) =>
                            setFP({ ...fP, motorista_id: v === "none" ? null : v })
                          }
                        >
                          <SelectTrigger>
                            <SelectValue placeholder="—" />
                      ...
                      <div className="space-y-1">
                        ...
                        <Input
                          ...
                          value={
                            fP.data_hora
                              ? fP.data_hora.slice(0, 16)
                              : new Date().toISOString().slice(0, 16)
                          }
                          onChange={(e) =>
                            setFP({ ...fP, data_hora: new Date(e.target.value).toISOString() })
                          }
                        />
                      </div>
                      <div className="space-y-1">
                        <Label>Rota / Rodovia</Label>
                        <Input
                          value={fP.rota ?? ""}
                          onChange={(e) => setFP({ ...fP, rota: e.target.value })}
                      ...
                      <div className="space-y-1">
                        ...
                        <Input
                          required
                          value={fP.praca ?? ""}
                          onChange={(e) => setFP({ ...fP, praca: e.target.value })}
                          placeholder="Ex: São Paulo - km 42"
                        />
                      </div>
                      <div className="space-y-1">
                        <Label>Valor *</Label>
                        <Input
                          ...
                          required
                          value={fP.valor ?? ""}
                          onChange={(e) => setFP({ ...fP, valor: e.target.value })}
                        />
                      </div>
                      <div className="space-y-1">
                        <Label>Pagamento</Label>
                        <Select
                          value={fP.forma_pagamento}
                          onValueChange={(v) => setFP({ ...fP, forma_pagamento: v })}
                        >
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value="tag">Tag automática</SelectItem>
                      ...
                      {fP.forma_pagamento === "tag" && (
                        <div className="space-y-1">
                          <Label>Operadora</Label>
                          <Select
                            value={fP.tag_operadora}
                            onValueChange={(v) => setFP({ ...fP, tag_operadora: v })}
                          >
                            <SelectTrigger>
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                              {OPERADORAS_TAG.map((o) => (
            ...
            <input
              ...
              accept=".csv,.txt"
              className="hidden"
              onChange={handleImportPedagio}
            />
            <Button variant="outline" size="sm" onClick={() => fileRef.current?.click()}>
              <Upload className="h-4 w-4" /> Importar CSV (Sem Parar/ConectCar/Veloe)
            </Button>
            <span className="text-xs text-muted-foreground">
              CSV com colunas: data, praça, valor, rota, placa (opcional)
            </span>
          </div>
          <Card>
            <div className="overflow-auto">
              <table className="w-full text-sm">
                <thead className="bg-muted/50 text-xs">
                  <tr>
                    ...
                    <th className="text-left p-2">Veículo</th>
                    <th className="text-left p-2">Motorista</th>
                    <th className="text-left p-2">Praça</th>
                ...
                <tbody>
                  {(pedagios as any[])
                    .filter(
                      (p) =>
                        !search ||
                        `${p.praca} ${p.rota ?? ""} ${p.veiculo?.placa ?? ""}`
                          .toLowerCase()
                          .includes(search.toLowerCase()),
                    )
                    .map((p) => (
                      <tr key={p.id} className="border-t">
                        ...
                        <td className="p-2">
                          <Badge variant="outline">
                            {p.forma_pagamento}
                            {p.tag_operadora ? ` · ${p.tag_operadora}` : ""}
                          </Badge>
                        </td>
                        <td className="p-2 text-right">
                          {canDelete && (
                            <Button
                              size="icon"
                              variant="ghost"
                              onClick={() => confirm("Excluir?") && removePedagio.mutate(p.id)}
                            >
                              <Trash2 className="h-3.5 w-3.5" />
                            </Button>
                          )}
              ...
              {(pedagios as any[]).length === 0 && (
                <p className="p-8 text-center text-sm text-muted-foreground">
                  Nenhum pedágio. Use o botão Importar CSV para carregar extratos de tag.
                </p>
              )}
            </div>
          </Card>
        ...
        <TabsContent value="motoristas" className="space-y-4 mt-4">
          <div className="flex gap-2 flex-wrap">
            {canEdit && (
              ...
              >
                ...
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>{editMotId ? "Editar motorista" : "Novo motorista"}</DialogTitle>
                  </DialogHeader>
                  <form
                    onSubmit={(e) => {
                      e.preventDefault();
                      saveMotorista.mutate();
                    }}
                    className="space-y-3"
                  >
                    <div className="space-y-1">
                      ...
                      <Input
                        required
                        value={fMot.nome ?? ""}
                        onChange={(e) => setFMot({ ...fMot, nome: e.target.value })}
                      />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        ...
                        <Input
                          value={fMot.cpf ?? ""}
                          onChange={(e) => setFMot({ ...fMot, cpf: e.target.value })}
                      ...
                      <div className="space-y-1">
                        <Label>CNH nº</Label>
                        <Input
                          value={fMot.cnh_numero ?? ""}
                          onChange={(e) => setFMot({ ...fMot, cnh_numero: e.target.value })}
                        />
                      </div>
                      <div className="space-y-1">
                        <Label>Categoria CNH</Label>
                        <Select
                          value={fMot.cnh_categoria}
                          onValueChange={(v) => setFMot({ ...fMot, cnh_categoria: v })}
                        >
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
                      ...
                      <div className="space-y-1">
                        ...
                        <Input
                          type="date"
                          value={fMot.cnh_validade ?? ""}
                          onChange={(e) =>
                            setFMot({ ...fMot, cnh_validade: e.target.value || null })
                          }
                        />
                      </div>
                      <div className="space-y-1">
                        ...
                        <Input
                          value={fMot.telefone ?? ""}
                          onChange={(e) => setFMot({ ...fMot, telefone: e.target.value })}
                        />
                      </div>
                      <div className="space-y-1">
                        <Label>Status</Label>
                        <Select
                          value={fMot.status}
                          onValueChange={(v) => setFMot({ ...fMot, status: v })}
                        >
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
                    ...
                    <div className="space-y-1">
                      <Label>Observações</Label>
                      <Textarea
                        value={fMot.observacoes ?? ""}
                        onChange={(e) => setFMot({ ...fMot, observacoes: e.target.value })}
                      />
                    </div>
                    <DialogFooter>
                      <Button type="submit">{editMotId ? "Atualizar" : "Cadastrar"}</Button>
                    </DialogFooter>
                  </form>
                </DialogContent>
              </Dialog>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base flex items-center gap-2">
                <Crown className="h-4 w-4 text-amber-500" /> Ranking de Eficiência
              </CardTitle>
              <CardDescription className="text-xs">
                Ordenado por maior km/L; desempate por menor custo total. Mostra vínculo real entre
                motorista e abastecimentos.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="overflow-auto">
                <table className="w-full text-sm">
                  <thead className="bg-muted/50 text-xs">
                    <tr>
                      ...
                      <th className="p-2 text-right">Abastec.</th>
                      <th className="p-2 text-right">km/L</th>
                      <th className="p-2 text-right">Combustível</th>
                      <th className="p-2 text-right">Avulsos</th>
                      <th className="p-2 text-right">Pedágios</th>
                  ...
                  <tbody>
                    {ranking
                      .filter(
                        (r) =>
                          !search || r.motorista.nome.toLowerCase().includes(search.toLowerCase()),
                      )
                      .map((r, idx) => {
                        ...
                        return (
                          <tr key={r.motorista.id} className="border-t">
                            ...
                            <td className="p-2 font-medium">
                              {r.motorista.nome}{" "}
                              <Badge
                                variant={r.motorista.status === "ativo" ? "default" : "secondary"}
                            ...
                            <td className="p-2">
                              ...
                              >
                                {r.motorista.cnh_categoria} · {s.label}
                              </Badge>
                            </td>
                            <td className="p-2 text-right">{r.totalAbast}</td>
                            <td className="p-2 text-right">
                              {r.mediaKml != null ? `${r.mediaKml.toFixed(2)}` : "—"}
                            </td>
                ...
                {ranking.length === 0 && (
                  <p className="p-6 text-center text-sm text-muted-foreground">
                    Cadastre motoristas e abastecimentos para ver o ranking.
                  </p>
                )}
              </div>
            </CardContent>
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {(motoristas as any[])
              .filter(
                (m) =>
                  !search ||
                  `${m.nome} ${m.cpf ?? ""}`.toLowerCase().includes(search.toLowerCase()),
              )
              .map((m) => {
                ...
                return (
                  <Card
                    key={m.id}
                    className={`p-4 ${s.nivel === "vencido" ? "border-rose-300" : s.nivel === "atencao" ? "border-amber-300" : ""}`}
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          CNH {m.cnh_categoria} {m.cnh_numero ? `· ${m.cnh_numero}` : ""}
                        </p>
                        <p
                          className={`text-xs mt-1 ${s.nivel === "vencido" ? "text-rose-600 font-semibold" : s.nivel === "atencao" ? "text-amber-600" : "text-muted-foreground"}`}
                        >
                          {s.nivel === "vencido" || s.nivel === "atencao" ? (
                      ...
                      <div className="flex gap-1">
                        ...
                        {canDelete && (
                          <Button
                            ...
                            variant="ghost"
                            onClick={() =>
                              confirm("Excluir motorista?") && removeMotorista.mutate(m.id)
                            }
                          >
                            <Trash2 className="h-4 w-4" />
                          </Button>
