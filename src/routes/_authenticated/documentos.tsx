...
function Browser({
  ...
  if (escopo === "obra" && !obraId) {
    return (
      <Card className="p-8 text-center text-muted-foreground">Selecione uma obra no topo.</Card>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2 text-sm flex-wrap">
          <button onClick={() => setPastaId(null)} className="text-primary hover:underline">
            Raiz
          </button>
          {breadcrumb.map((p) => (
            <span key={p.id} className="flex items-center gap-1">
              <ChevronRight className="h-3 w-3 text-muted-foreground" />
              <button onClick={() => setPastaId(p.id)} className="text-primary hover:underline">
                {p.nome}
              </button>
            </span>
          ))}
              ...
              <DialogContent>
                ...
                <DialogFooter>
                  <Button
                    onClick={() => {
                      if (novoNome.trim()) {
                        createPasta.mutate(novoNome.trim());
                        setNovoNome("");
                        setNovaOpen(false);
                      }
                    }}
          ...
          >
            ...
            {canDelete && (
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  if (confirm(`Excluir pasta "${p.nome}" e todo o conteúdo?`))
                    removePasta.mutate(p.id);
                }}
              >
            ...
            <div className="flex items-center gap-2 min-w-0 flex-1">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  {safeFormatDate(d.created_at, "dd/MM/yyyy HH:mm")} ·{" "}
                  {((d.tamanho ?? 0) / 1024).toFixed(1)} KB
                </p>
              </div>
            </div>
            <div className="flex gap-1">
              <Button size="icon" variant="ghost" onClick={() => downloadDoc(d)} title="Baixar">
              ...
              {canDelete && (
                <Button
                  ...
                  variant="ghost"
                  onClick={() => {
                    if (confirm("Excluir arquivo?")) removeDoc.mutate(d);
                  }}
                >
