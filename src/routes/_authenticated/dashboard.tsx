...
function DashboardPage() {
  ...
  const indicadores = [
    ...
    },
    {
      ...
      icon: CheckSquare,
      status: tarefas.length > 10 ? "vermelho" : tarefas.length > 0 ? "amarelo" : "verde",
    },
    {
      label: "Contas a pagar",
      valor: contasPagar.length,
      icon: Wallet,
      status: contasPagar.length === 0 ? "verde" : "amarelo",
    },
    {
      label: "EPIs abaixo do mín.",
      valor: epiAbaixoMin.length,
      icon: HardHat,
      status: epiAbaixoMin.length === 0 ? "verde" : "vermelho",
    },
    {
      label: "Materiais abaixo do mín.",
      valor: matAbaixoMin.length,
      icon: Package,
      status: matAbaixoMin.length === 0 ? "verde" : "vermelho",
    },
  ] as const;
    ...
    <div className="space-y-6">
      ...
      {isAdmin(roles) && (
        <Card className="p-3 flex items-center justify-between bg-primary/5 border-primary/30">
          <div className="flex items-center gap-2 text-sm">
            <ShieldCheck className="h-4 w-4 text-primary" /> Você é administrador.
          </div>
          <Link to="/acessos" className="text-sm underline">
            Gerenciar acessos
          </Link>
        </Card>
      )}
            ...
            <>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left text-xs text-muted-foreground border-b">
                      ...
                      {VENC_FIELDS.map((f) => (
                        <th key={f.key} className="py-2 pr-3 whitespace-nowrap">
                          {f.label}
                        </th>
                      ))}
                    </tr>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 mt-3 text-sm">
                <span className="text-muted-foreground">
                  Página {pageSafe} de {totalPages} · {sorted.length} registro
                  {sorted.length === 1 ? "" : "s"}
                </span>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base">
              <Package className="h-4 w-4" /> Estoque abaixo do mínimo
            </CardTitle>
          </CardHeader>
          <CardContent>
            {epiAbaixoMin.length + matAbaixoMin.length === 0 ? (
              ...
              <ul className="divide-y">
                {epiAbaixoMin.map((e: any) => (
                  <li key={e.id} className="flex items-center justify-between py-2 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-muted-foreground">(EPI)</span>
                    </div>
                ...
                {matAbaixoMin.map((m: any) => (
                  <li key={m.id} className="flex items-center justify-between py-2 text-sm">
                    ...
                    <span className="text-xs text-destructive">
                      {Number(m.estoque_atual).toFixed(2)} / mín.{" "}
